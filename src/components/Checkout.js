...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      const  resp  =await axios.post(`${config.endpoint}/user/addresses`,
    ...
    setAddresses({ ...addresses, all: resp.data });
  ...
  const totalpayamount = getTotalCartValue(items) +
  (getTotalCartValue(items) > 1000
    ? 0
    : Number(
        (getTotalCartValue(items) * 0.075).toFixed()
      ));
  ...
  const validateRequest = (items, addresses) => {
     ...
     if(localStorage.getItem("balance")< totalpayamount)
     {
      enqueueSnackbar("You do not have enough balance",{variant:"warning"});
      return false;
     }

     if(!addresses.all.length)
     {
      enqueueSnackbar("Please add address",{variant:"warning"});
      return false;
     }
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    'Authorization': `Bearer ${token}`
     ...
     const newbalance = localStorage.getItem("balance")-totalpayamount;
    ...
    history.push("/thanks");
      ...
      if (e.response) {
        ...
        enqueueSnackbar(
          "Could not Place your Order. Check that the backend is running, reachable and returns valid JSON.",
          {
            variant: "error",
          }
        );
      ...
      <Grid container>
        ...
        {    newAddress.isAddingNewAddress ? 
            ...
            <Typography color="#3C3C3C" my="1rem" variant="h6">
              Payment Methods Accepted - UPI,Cash On Delivery (COD),Pay on Delivery (POD)
            </Typography>
            <Divider />

            <Box my="1rem">
            <Typography color="#3C3C3C" variant="h5" my="1rem">Wallet</Typography>
            <Typography color="#3C3C3C" my="1rem" variant="h6">
                Pay $
                {totalpayamount}{" "}
            ...
            <Button startIcon={<CreditCard />} variant="contained" onClick={async()=>{await performCheckout(token,items,addresses)}}>
              PLACE ORDER
            </Button>
          </Box>
        </Grid>
